...
public class ServiceObjetMetier {
    ...
    public void rechercherPersonneParNom(String nom) throws ServiceException {
        try {
            List<Object[]> listePersonneIds = this.dBConnection.launchQuery("SELECT PersonneID, Nom, Prenom, Mail FROM PERSONNE WHERE Nom LIKE ? ORDER BY PersonneID", "%"+nom.toUpperCase()+"%");

            JsonArray jsonListe = new JsonArray();

            for (Object[] row : listePersonneIds) {
                JsonObject jsonItem = new JsonObject();
